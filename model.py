...
def build_token_to_id_vocab(sentences, specials=('<pad>', '<bos>', '<eos>', '<unk>')):
    ...
    token_id_dict = {item:idx for idx ,item in enumerate(specials)}

    next_id = len(token_id_dict)
    for item in sentences:
        for token in item.split(" "):
            if token not in token_id_dict:
                token_id_dict[token] = next_id
                next_id +=1
    
    return token_id_dict

# Step 2 - build_id_to_token_vocab
def build_id_to_token_vocab(token_to_id: dict):
...
def decode_ids_to_tokens(ids, id_to_token):
    ...
    return [id_to_token[k] for k in ids]
...
def split_last_dim_into_heads(tensor, num_heads):
    ...
    return tensor.reshape(B, L, num_heads, d_model)
